/**
 * Sidebar.jsx — Hover-expand desktop sidebar shell
 *
 * Pass your nav items as props. Define them in your feature area
 * once you know what pages you need.
 *
 * Design: Sidebar Glass spec (theme.md §5.3)
 * Collapsed: 80px  |  Expanded (on hover): 260px
 *
 * Usage:
 *   const NAV = [
 ...
 *   ]
 *   <Sidebar nav={NAV} appName="TransitOps" user={currentUser} onLogout={handleLogout} />
 */
...
/**
 * @param {{
 *   nav?: Array<{ label: string, path: string, icon: React.ReactNode, section?: string }>,
 *   appName?: string,
 *   user?: { name: string, email: string, role: string } | null,
 *   onLogout?: () => void,
 * }} props
 */
export function Sidebar({ nav = [], appName = 'TransitOps', user = null, onLogout }) {
      ...
      <div className="flex items-center gap-3 px-[22px] py-5 shrink-0">
        ...
        {expanded && (
          <div className="overflow-hidden whitespace-nowrap">
            ...
            {user?.role && (
              <p className="text-[11px] text-text-secondary mt-0.5 capitalize">{user.role.replace('_', ' ')}</p>
            )}
          </div>
        )}
      </div>
        ...
        {nav.map((item) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute left-0 inset-y-2 w-[3px] rounded-r bg-ink-500" aria-hidden="true" />
                )}
