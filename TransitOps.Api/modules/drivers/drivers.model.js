...
export const getDocuments = async (driverId) => {
    const query = `SELECT id, doc_type as "docType", file_path as "filePath", expiry_date as "expiryDate", is_verified as "isVerified", created_at as "createdAt" FROM driver_documents WHERE driver_id = $1 ORDER BY created_at DESC`;
    const { rows } = await db.query(query, [driverId]);
    return rows;
};

export const addDocument = async (driverId, docType, filePath, expiryDate) => {
    const query = `INSERT INTO driver_documents (driver_id, doc_type, file_path, expiry_date) VALUES ($1, $2, $3, $4) RETURNING id, doc_type as "docType", file_path as "filePath", expiry_date as "expiryDate", is_verified as "isVerified", created_at as "createdAt"`;
...
export const updateDocumentVerification = async (driverId, docId, isVerified) => {
    const query = `UPDATE driver_documents SET is_verified = $1 WHERE id = $2 AND driver_id = $3 RETURNING id, is_verified as "isVerified"`;
    const { rows } = await db.query(query, [isVerified, docId, driverId]);
    return rows[0];
};

export const isAllDocumentsVerified = async (driverId) => {
    const query = `
        SELECT 
            COUNT(*) as total_docs,
